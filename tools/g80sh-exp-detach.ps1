# g80sh-exp-detach.ps1 — 티켓 21. 화면이 꺼진 동안 G80SH 를 화면 구성에서 분리하면(G50F 만 활성) 폭풍이 멎는가, 그때 GPU D3 는 살아 있는가.
#
...
$etl = Join-Path $d 'g80sh-exp-detach.etl'
try {
    $w0 = Get-Date
    ...
    $r = [Ccd2]::Activate([string[]]@($G50), [int[]]@(0), [int[]]@(0))
    powercfg /setactive SCHEME_CURRENT
    Start-Sleep -Seconds 5
    $t1 = Get-Date; Log ("G80SH 분리: $r → layout: $((Get-Layout).Active -join ',') / 전원 설정 재적용")
    for ($i = 1; $i -le $Min; $i++) {
        Start-Sleep -Seconds 60
        Log ("  +{0}분 사이클 누적 {1}  layout {2}" -f $i, (Cyc $t1), ((Get-Layout).Active -join ','))
    }
    Log ("분리 {0}분: 사이클 {1}" -f $Min, (Cyc $t1))
    # 입력 복귀 확인
    $tw = Get-Date
    ...
    Unregister-ScheduledTask -TaskName 'G80SH Exp Detach' -Confirm:$false -ErrorAction SilentlyContinue
}
